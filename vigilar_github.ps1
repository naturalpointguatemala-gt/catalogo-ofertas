...
$SourceFolder="C:\Users\Natural Point Progra\OneDrive\NATURAL POINT\OFERTAS"
$ImagesDir=Join-Path $RepoRoot "images"
$Ext=@(".jpg",".jpeg",".png",".gif",".webp",".bmp",".tif",".tiff",".heic")
if(-not(Test-Path $SourceFolder)){Write-Host "No se encontro la carpeta OFERTAS." -ForegroundColor Red;pause;exit 1}
function Sig{
...
function Build{
 ...
 New-Item -ItemType Directory $ImagesDir|Out-Null;$out=@()
 Get-ChildItem $SourceFolder -File -Recurse|Where-Object{$Ext -contains $_.Extension.ToLower()}|ForEach-Object{
  ...
  $td=if($dir){Join-Path $ImagesDir $dir}else{$ImagesDir};New-Item -ItemType Directory $td -Force|Out-Null
...
function Publish{
 ...
 try{
  ...
  & git commit -m ("Actualizar catalogo ofertas "+(Get-Date -Format "yyyy-MM-dd HH:mm:ss"))
  & git push origin main
  if($LASTEXITCODE -eq 0){Write-Host "PUBLICACION COMPLETADA." -ForegroundColor Green}
 }finally{Pop-Location}
}
Write-Host ("Vigilando OFERTAS: "+$SourceFolder) -ForegroundColor Green
...
while($true){
 ...
 if($pending -and ((Get-Date)-$lastChange).TotalSeconds -ge 60){$pending=$false;Build;Publish;Write-Host "Vigilancia activa - OFERTAS..."}
}
